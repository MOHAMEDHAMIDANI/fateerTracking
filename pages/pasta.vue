        ...
        <div class="flex flex-col items-center w-full">
            <div class="flex items-center justify-around w-full h-16 my-5">
                <select class="outline-none w-[200px] h-9 rounded text-2xl capitalize bg-gray-100" name="meals"
                    ...
                    <option value="Couscous / Pasta">Couscous / Pasta</option>
...
const type = ref('Flour')
const Qnt = ref('')
const Price = ref('')
let pasta = []; // Initialize sugar as a ref with an empty array

onBeforeMount(() => {
    const storedpasta = localStorage.getItem('pasta');
    pasta = storedpasta ? JSON.parse(storedpasta) : []; // Assign the parsed value to sugar.value
});
...
const addToTable = () => {
    ...
    pasta.push(temporary); // Access sugar as a ref using sugar.value
