        ...
        <div class="flex flex-col items-center w-full">
            <div class="flex items-center justify-around w-full h-16 my-5">
                <select class="outline-none w-[200px] h-9 rounded text-2xl capitalize bg-gray-100" name="meals"
                    ...
                    <option value="Roumi cheese">Roumi cheese</option>
                    <option value="Kiri cheese">Kiri cheese</option>
                    <option value="Cheddar cheese">Cheddar cheese</option>
                    <option value="Cheddar cheese">Cream cheese</option>
...
const Qnt = ref('')
const Price = ref('')
let milk = []; // Initialize sugar as a ref with an empty array

onBeforeMount(() => {
    const storedmilk = localStorage.getItem('milk');
    milk = storedmilk ? JSON.parse(storedmilk) : []; // Assign the parsed value to sugar.value
});
...
const addToTable = () => {
    ...
    milk.push(temporary); // Access sugar as a ref using sugar.value
