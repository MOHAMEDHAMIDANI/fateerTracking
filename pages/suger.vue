        ...
        <div class="flex flex-col items-center w-full">
            <div class="flex items-center justify-around w-full h-16 my-5">
                <select class="outline-none w-[200px] h-9 rounded text-2xl capitalize bg-gray-100" name="meals"
                    ...
                    <option value="Sugar powder">Sugar powder</option>
                </select>
...
const type = ref('Custard')
const Qnt = ref('')
const Price = ref('')
let sugar = []; // Initialize sugar as a ref with an empty array

onBeforeMount(() => {
    const storedSugar = localStorage.getItem('sugar');
    sugar = storedSugar ? JSON.parse(storedSugar) : []; // Assign the parsed value to sugar.value
});
...
const addToTable = () => {
    ...
    sugar.push(temporary); // Access sugar as a ref using sugar.value
